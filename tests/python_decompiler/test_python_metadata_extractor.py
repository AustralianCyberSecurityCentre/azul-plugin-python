...
class PyDecTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.pyc_valid = os.path.join(os.path.dirname(__file__), test_precompiled_py12_valid)
        cls.pyc_valid_header = os.path.join(os.path.dirname(__file__), test_precompiled_py12_valid_header)
    ...
    def test_metadata_valid_pyc(self):
        ...
        metadata = extract_metadata(self.pyc_valid)
    ...
    def test_metadata_valid_pyc_header_only(self):
        """Test decompilers extract_metadata against an invalid file, with correct headers."""
        metadata = extract_metadata(self.pyc_valid_header)

        self.assertEqual(metadata["error_type"], DecompileErrors.XdisGetCode)
        self.assertEqual(metadata["error_msg"], DecompileErrors.XdisGetCodeMsg)
