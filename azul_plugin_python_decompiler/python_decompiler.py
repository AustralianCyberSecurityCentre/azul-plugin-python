...
# in an attempt to avoid the lag between xdis updating with supporting new patch
# releases of python, we add our own 3.9 mappings up to our current interpreter
if sys.version_info[:2] == (3, 9):
    import xdis.magics

    bytecode_version = "3.9.0beta5"
    versions = ["3.9"]
    versions.extend([f"3.9.{i}" for i in range(sys.version_info[2] + 1)])
    versions.append("3.9.0b5+")
    xdis.magics.add_canonic_versions(" ".join(versions), "3.9.0beta5")
